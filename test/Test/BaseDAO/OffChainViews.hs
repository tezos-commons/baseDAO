...
test_FA2 =
  [ testGroup "is_operator" $
    let checkOperator st param =
          runView @Bool DAO.isOperatorView st (ViewParam param)
    in
    ...
    , testCase "Present operator" $
        checkOperator
          defaultStorage
            { DAO.sOperators = BigMap $ one ((#owner .! addr1, #operator .! addr2), ()) }
          FA2.OperatorParam
            { opOwner = addr1, opOperator = addr2, opTokenId = FA2.theTokenId }
        @?= Right True
  ...
  , testGroup "token_metadata" $
    [ testCase "Get frozen tokens metadata" $
        runView @(FA2.TokenId, FA2.TokenMetadata)
          DAO.tokenMetadataView defaultStorage (ViewParam DAO.frozenTokenId)
