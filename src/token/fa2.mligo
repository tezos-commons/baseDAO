...
let transfer_item (store, ti : storage * transfer_item): storage =
  let transfer_one (store, tx : storage * transfer_destination): storage =
    ...
    if tx.amount = 0n then store
      ...
      let valid_token_id =
        if tx.token_id = store.frozen_token_id then (
...
[@inline]
let validate_token_type (token_id, frozen_token_id : token_id * nat): token_id =
  if (token_id = frozen_token_id) then
    token_id
...
let balance_of (params, store : balance_request_params * storage): return =
  let check_one (req : balance_request_item): balance_response_item =
    let valid_token_id = validate_token_type(req.token_id, store.frozen_token_id) in
    let bal =
...
// -----------------------------------------------------------------
[@inline]
let validate_operator_token (token_id, frozen_token_id, is_operator_admin : token_id * token_id * bool): token_id =
  if (token_id = frozen_token_id) then
    // it seems that Michelson won't allow always failing code, so have included this branch
    if is_operator_admin
        then token_id
        else (failwith("OPERATION_PROHIBITED") : token_id)
  else
...
let update_one (store, param: storage * update_operator): storage =
  ...
  in
  let valid_token_id = validate_operator_token (operator_param.token_id, store.frozen_token_id, store.admin = operator_param.operator) in
