...
checkProposalLimitReached :: ModelT cep ()
...
stakeTk :: Natural -> Address -> ModelT cep ()
...
removeFromProposalSortByLevel :: Natural -> ProposalKey -> ModelT cep ()
...
applyPropose :: ModelSource -> ProposeParams -> ModelT cep ()
...
handleProposalIsOver :: forall var. (Natural, ProposalKey) -> ModelT var Bool
handleProposalIsOver (proposalLvl, proposalKey) = do
  ...
  if isExpired then
    ...
    when cond $ do
      store :: StorageSkeleton (VariantToExtra var) <- getStore @var
      (ops, newExtra, guardianMaybe) <-
        (msDecisionLambda <$> get) >>= \f -> f (DecisionLambdaInput' proposal (sExtra store) :: DecisionLambdaInput' (VariantToExtra var))

      case guardianMaybe of
        Just g -> modifyStore $ \s -> pure $ s { sGuardian = g }
        Nothing -> pure ()

      modifyStore @var $ \s -> pure $ s { sExtra = newExtra }
